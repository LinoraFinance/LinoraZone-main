...
```
docker build -t linora-market-maker .
```
...
```
conda create -n linora-mm python=3.14
conda activate linora-mm
```
...
To authenticate with linora, you need to provide your Wallet address and private key. You can get more information in the [linora API documentation](https://docs.linora.trade/developer-portal/general-information/api-quick-start).

You can then set the `linora_L1_ADDRESS` and `linora_PRIVATE_KEY` environment variables. See below for more details. You will also need to set the `linora_ENVIRONMENT` environment variable to `mainnet` or `testnet`.

Example:
```
export linora_L1_ADDRESS="0x1234567890123456789012345678901234567890"
export linora_PRIVATE_KEY="0x1234567890123456789012345678901234567890123456789012345678901234"
export linora_ENVIRONMENT="testnet"
```
...
```
docker run -e PARAM_NAME=value linora-market-maker
```
