"""
This module implements a connector for the linora perpetual exchange.
...
from linora_py.common.order import Order as linoraOrder, OrderSide as linoraOrderSide, OrderType as linoraOrderType
from linora_py.environment import PROD, TESTNET
...
class linoraPerpConnector(ConnectorBase):
    """
    A connector class for interacting with the linora perpetual exchange.

    This class provides methods for order management, market data subscriptions,
    and account information retrieval.

    Attributes:
        linora (linora): The linora client instance.
        exchange (str): The name of the exchange ("linora_perp").
    ...
    Methods:
        ...
        initialize(): Initialize the linora client.
    ...
    def __init__(self, loop, key=None, secret=None):
        """
        Initialize the linoraPerpConnector.
        ...
        self.linora: linora = None
        self.exchange = "linora_perp"

        self.l1_address = key or os.getenv("linora_L1_ADDRESS")
        self.l2_private_key = secret or os.getenv("linora_PRIVATE_KEY")
        ...
        self.rate_limiter = SyncRateLimiter(int(os.getenv("linora_RATE_LIMIT", "8"))) # X requests per second, adjust as needed

    def rate_limited_request(self, method: Callable, *args, **kwargs) -> Any:
        """
        Rate-limit the request to the linora API.
        """
    ...
    def get_account_str(self):
        ...
        return str(self.linora.account.l2_address)

    async def initialize(self):
        """Initialize the linora client with environment-specific settings."""
        _para_env = os.getenv("linora_ENVIRONMENT")
        _linora_perpetual_chain = PROD if _para_env == "mainnet" else (TESTNET if _para_env == "testnet" else _para_env)

        self.linora = linora(
            env=_linora_perpetual_chain, 
    ...
    def cancel_order(self, client_order_id, by="exchange_order_id"):
        ...
        """            
        cancel_via_client_order_id = self.linora.api_client.cancel_order_by_client_id
        if by == 'client_order_id':
            ...
            cancel_method = self.linora.api_client.cancel_order
    ...
    def sync_open_orders(self, mkt):
        ...
        existing_orders = []
        try:
            p_orders = self.rate_limited_request(self.linora.api_client.fetch_orders, {'market': mkt})
        ...
        for ao in set(self.active_orders.keys()) - set(already_processed):
            try:
                po = self.rate_limited_request(self.linora.api_client.fetch_order_by_client_id, ao)
                self.process_order_update(po)
            except ValidationError as ve:
                if ve.data['message'] == 'rate limit exceeded':
                    self.logger.warning(f"rate limit exceeded")
    ...
    def cancel_all_orders(self, mkt):
        ...
        Args:
            mkt (str): The market symbol.
        """
        self.rate_limited_request(self.linora.api_client.cancel_all_orders, {"market":mkt})
    ...
    def insert_order(self, order: Order):
        ...
        if order.order_type == OrderType.LIMIT_MAKER:
            _order_type = linoraOrderType.Limit
            _order_instruction = "POST_ONLY"
        elif order.order_type == OrderType.LIMIT:
            _order_type = linoraOrderType.Limit
            _order_instruction = "GTC"
        elif order.order_type == OrderType.MARKET:
            _order_type = linoraOrderType.Market
            _order_instruction = "IOC"
        elif order.order_type == OrderType.IOC:
            _order_type = linoraOrderType.Limit
            _order_instruction = "IOC"

        _order_side = linoraOrderSide.Buy if order.side == Side.BUY else linoraOrderSide.Sell
        ...
        try:
            resp = self.rate_limited_request(self.linora.api_client.submit_order, _po)
        except Exception as e:
    ...
    async def _connect_to_ws(self):
        ...
        while not is_connected:
            is_connected = await self.linora.ws_client.connect()
            if not is_connected:
    ...
    def fetch_bbo(self, symbol: str) -> Dict[str, Level]:
        bbos = self.linora.api_client.fetch_bbo(symbol)
        return {
    ...
    async def subscribe_to_data_channels(self, market, callback):
        ...
        await self.linora.ws_client.subscribe(
        ...
        await self.linora.ws_client.subscribe(
            linoraWebsocketChannel.ORDER_BOOK_DELTAS,
    ...
    async def setup_trading_rules(self, market):
        ...
        Args:
            market (str): The market symbol.
        """
        _para_markets = self.linora.api_client.fetch_markets()
    ...
    async def snapshots(self):
        ...
        positions = self.linora.api_client.fetch_positions()
