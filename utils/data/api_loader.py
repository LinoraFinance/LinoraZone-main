...
from linora_py.environment import PROD
...
@dataclass
class DataConfig:
    """Configuration for linora data operations"""
    client: linoraApiClient = linoraApiClient(env=PROD)
    base_dir: Path = Path("data/linora")
...
def retry_fetch(func: Callable) -> Callable:
    """Wrapper to add exponential backoff retries for linora fetch API calls"""
...
linoraApiClient.fetch_markets = retry_fetch(linoraApiClient.fetch_markets)
linoraApiClient.fetch_markets_summary = retry_fetch(linoraApiClient.fetch_markets_summary)

def load_trades_market(market: str, start_at: str = None, end_at: str = None, data_config: DataConfig = DataConfig()):
    """Load trades for a specific market from linora API and save to parquet files"""
...
def load_trades(data_config: DataConfig = DataConfig()):
    """Load all trades from linora API for each market"""
    data_config.mkdirs()
...
def load_markets_summary(data_config: DataConfig = DataConfig()):
    """Load market summary data for all available markets from linora API"""
    data_config.mkdirs()
    markets = duckdb.query(f"select symbol from '{data_config.base_dir}/markets.parquet' order by symbol").df()
    for market in markets['symbol']:
...
def load_markets(data_config: DataConfig = DataConfig()):
    """Load markets data from linora API and save to parquet file"""
