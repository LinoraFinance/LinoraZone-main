...
from linora_py.common.order import (
    Order as linoraOrderObject,
    OrderSide as linoraOrderSide,
    OrderType as linoraOrderType,
)

from hummingbot.connector.constants import s_decimal_NaN
from hummingbot.connector.derivative.linora_perpetual import (
    linora_perpetual_constants as CONSTANTS,
    linora_perpetual_web_utils as web_utils,
)
from hummingbot.connector.derivative.linora_perpetual.linora_perpetual_api_order_book_data_source import (
    linoraPerpetualAPIOrderBookDataSource,
)
from hummingbot.connector.derivative.linora_perpetual.linora_perpetual_auth import linoraPerpetualAuth
from hummingbot.connector.derivative.linora_perpetual.linora_perpetual_user_stream_data_source import (
    linoraPerpetualUserStreamDataSource,
)
...
class linoraPerpetualDerivative(PerpetualDerivativePyBase):
    web_utils = web_utils
    ...
    def __init__(
        ...
        self.linora_perpetual_l1_address = linora_perpetual_l1_address
        self.linora_perpetual_is_testnet = linora_perpetual_is_testnet
        self.linora_perpetual_l1_private_key = linora_perpetual_l1_private_key
        self.linora_perpetual_l2_private_key = linora_perpetual_l2_private_key
    ...
    @property
    def name(self) -> str:
        # Note: domain here refers to the entire exchange name. i.e. linora_perpetual or linora_perpetual_testnet
        return self._domain

    @property
    def authenticator(self) -> linoraPerpetualAuth:
        return linoraPerpetualAuth(
                self.linora_perpetual_l1_address,
                self.linora_perpetual_is_testnet,
                self.linora_perpetual_l1_private_key,
                self.linora_perpetual_l2_private_key)
    ...
    async def _initialize_trading_pair_symbol_map(self):
        try:
            self.logger().info("Retrieving trading pairs from linora Perpetuals...")
            exchange_info = await self._api_get(path_url=self.trading_pairs_request_path,
                                                 data={})

            self.logger().info("got trading pairs from linora Perpetuals...")
            self._initialize_trading_pair_symbols_from_exchange_info(exchange_info=exchange_info)
        except Exception:
            self.logger().exception("There was an error requesting exchange info.")

    def _create_order_book_data_source(self) -> OrderBookTrackerDataSource:
        return linoraPerpetualAPIOrderBookDataSource(
    ...
    def _create_user_stream_data_source(self) -> UserStreamTrackerDataSource:
        return linoraPerpetualUserStreamDataSource(
            auth=self._auth,
            trading_pairs=self._trading_pairs,
            connector=self,
            api_factory=self._web_assistants_factory,
    ...
    async def _place_cancel(self, order_id: str, tracked_order: InFlightOrder):
        ...
        self.logger().debug(f"The order {order_id} does not exist on linora Perpetuals. "
    ...
    async def _place_order(
        ...
        param_order_type = linoraOrderType.Limit
        param_order_tif = "GTC"
        if order_type == OrderType.LIMIT_MAKER:
            param_order_tif = "POST_ONLY"
        elif order_type == OrderType.MARKET:
            param_order_type = linoraOrderType.Market

        side_map = {
            TradeType.BUY: linoraOrderSide.Buy,
            TradeType.SELL: linoraOrderSide.Sell
        }

        self.logger().info(f"Creating order tif: {param_order_tif}")
        order_object = linoraOrderObject(
            stp="EXPIRE_BOTH",
            client_id=order_id,
            market=symbol,
        ...
        order_object.signature = self._auth.linora_account.sign_order(order_object)
        ...
        while True:
            try:
                ...
                self.logger().network(
                    ...
                    exc_info=True,
                    app_warning_msg="Could not fetch user events from linora. Check API key and network connection.",
                )
    ...
    async def _trading_pair_position_mode_set(self, mode: PositionMode, trading_pair: str) -> Tuple[bool, str]:
        ...
        if initial_mode != mode:
            msg = "linora only supports the ONEWAY position mode."
            success = False
        else:
            ...
            self.logger().debug(f"linora switching position mode to " f"{mode} for {trading_pair} succeeded.")
        return success, msg

    async def _set_trading_pair_leverage(self, trading_pair: str, leverage: int) -> Tuple[bool, str]:
        # TODO
        msg = "Leverage setting is not supported by linora."
        success = True
        return success, msg
